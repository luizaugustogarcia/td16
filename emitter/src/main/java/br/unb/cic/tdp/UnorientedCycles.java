 ...
 * sequence using a GPU-accelerated search.
 */
@Slf4j
public class UnorientedCycles {
    ...
    public static void processPartitions(final int[][] partitions, final int target, final int maxMoves) {
        val minRate = 2 / (maxMoves / (float) target);
        for (val partition : partitions) {
            processPartition(partition, minRate, target, maxMoves);
        }
    }
    ...
    private static void processPartition(final int[] partition,
        ...
        DirectTwistedBraceletGenerator.generateRepresentativesConcurrently(
                partition, new boolean[partition.length], () -> { }, pair -> {
                    val moves = SortingSearch.searchForSorting(pair, minRate, maxMoves);
                    if (moves.isEmpty()) {
                        throw new IllegalStateException(unsolvedMessage(
                                partition, target, maxMoves, pair));
                    }
