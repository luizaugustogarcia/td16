...
public class MulticyclePermutation implements Collection<Cycle>, Permutation {
    ...
    public static final byte[][] CANONICAL_BETA_BYTE;

    static {
        CANONICAL_BETA_BYTE = new byte[2000][];
        for (var i = 1; i < 2000; i++) {
            val beta = new int[i];
            val b_beta = new byte[i];
            for (var j = 0; j < i; j++) {
                beta[j] = j;
                b_beta[j] = (byte) j;
            }
            CANONICAL_BETA_BYTE[i] = b_beta;
        }
    }
    ...
    public MulticyclePermutation(final int expectedCycleCount,
                                 final int expectedSymbolCount) {
        cycles = new ArrayList<>(expectedCycleCount);
        symbols = new HashSet<>(Math.max(1,
                (int) Math.ceil(expectedSymbolCount / 0.75)));
    }
    ...
    public static byte[] oneLineNotation(final Collection<Cycle> omega) {
        // assumption: beta is always CANONICAL_BETA_BYTE i.e. omega is in standard form
        val numberOfSymbols = omega.stream().mapToInt(Cycle::size).sum();
        val result = Arrays.copyOf(CANONICAL_BETA_BYTE[numberOfSymbols], numberOfSymbols);
        for (val cycle : omega) {
    ...
    public MulticyclePermutation(final Collection<Cycle> cycles) {
        this(cycles.size(), cycles.stream().mapToInt(Cycle::size).sum());
        this.addAll(cycles);
    }
