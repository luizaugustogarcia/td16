...
final class DirectTwistedBraceletGenerator {
    ...
    private static final String GPU_DEVICES_PROPERTY = "tdp.gpuDevices";
    private static final String GPU_SLOTS_PROPERTY = "tdp.gpuSlots";
    private static final int DEFAULT_WORKER_COUNT = defaultWorkerCount();
    ...
    private static int defaultWorkerCount() {
        final int slotsPerDevice = Integer.parseInt(
                System.getProperty(GPU_SLOTS_PROPERTY, "7"));
        final int deviceCount = GpuDeviceLayout.parseDevices(
                System.getProperty(GPU_DEVICES_PROPERTY, "0")).length;
        return defaultWorkerCount(Runtime.getRuntime().availableProcessors(),
                slotsPerDevice, deviceCount);
    }

    static int defaultWorkerCount(final int availableProcessors,
                                  final int slotsPerDevice,
                                  final int deviceCount) {
        return Math.addExact(availableProcessors,
                Math.multiplyExact(slotsPerDevice, deviceCount));
    }
