...
public class Oriented9CycleExtensions {
    ...
    /**
     * Starts each certificate lookup in a virtual thread.
     */
    ...
    record SearchRequirement(int maxMoves, int requiredIncrease) {
    }
    ...
    /**
     * Loads and verifies the required (9,x)-sequence for the given pair.
     *
    ...
    private static List<Cycle> searchForSorting(final CyclicTargetPair pair,
                                                final SearchRequirement requirement) {
        return SortingSearch.searchForSorting(pair.canonicalRepresentative(), requirement.maxMoves());
    }
